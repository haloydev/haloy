...
func (u *Updater) Update(ctx context.Context, logger *slog.Logger, reason TriggerReason, app *TriggeredByApp) (UpdateResult, error) {
	...
	// Step 1: Discover all containers with haloy labels
	...
	// Step 2: Health check all discovered containers
	...
	// Combine all failed containers for result
...
// logFailedContainers logs information about containers that failed during a specific phase.
func logFailedContainers(failed []FailedContainer, logger *slog.Logger, phase string) {
	for _, f := range failed {
		if f.Labels != nil {
			logger.Error(fmt.Sprintf("Container failed %s: %s", phase, f.Reason),
